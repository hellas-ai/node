{self}: rec {
  ...
  in
    ...
    ++ lib.concatMap (addr: ["--node-addr" addr]) gateway.nodeAddrs
    ...
    ++ ["--responses-backend" gateway.responsesBackend]
    ++ ["--responses-proxy-url" gateway.responsesProxyUrl]
    ++ ["--responses-proxy-api-key-env" gateway.responsesProxyApiKeyEnv]
    ++ gateway.extraArgs;
}
