...
//!
//! NOTE: the generated dispatcher currently passes only the request
//! body to handlers. Until peer identity is threaded into handler
//! context, peer-aware filtering in `get_known_peers` falls back to an
//! anonymous requester (`PeerId::default()`) and
//! `min_disclosed_auth_level` on `PeerDirectoryConfig` is the gate.
...
impl NodeHandler for NodeHandlerImpl {
    ...
    async fn get_known_peers(
        ...
        // Anonymous requester until peer identity is available in the
        // handler context. `min_disclosed_auth_level` on the directory
