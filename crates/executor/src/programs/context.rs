...
use hellas_runtime::runtime::{
    BoundProgramText, TextExecution, TextPolicy, TextReceipt, TextState,
};
...
impl ExecutionCache {
    ...
    fn insert_receipt(
        ...
        if self.receipts.contains_key(&receipt_id) {
            // Same content, already present; receipt cache entries are
            // not refreshed on duplicate insert.
            return;
        }
