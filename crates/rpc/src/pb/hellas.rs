...
pub mod quote_client {
    ...
    use tonic::codegen::http::Uri;
    #[derive(Debug, Clone)]
    pub struct QuoteClient<T> {
        inner: tonic::client::Grpc<T>,
    }
    impl QuoteClient<tonic::transport::Channel> {
        /// Attempt to create a new client by connecting to a given endpoint.
        pub async fn connect<D>(dst: D) -> Result<Self, tonic::transport::Error>
        where
            D: TryInto<tonic::transport::Endpoint>,
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> QuoteClient<InterceptedService<T, F>>
        where
            F: tonic::service::Interceptor,
            T::ResponseBody: Default,
            T: tonic::codegen::Service<
            ...
            >>::Error: Into<StdError> + std::marker::Send + std::marker::Sync,
        {
            QuoteClient::new(InterceptedService::new(inner, interceptor))
        }
        /// Compress requests with the given encoding.
        ///
        /// This requires the server to support it otherwise it might respond with an
        ...
        > {
            ...
            let codec = tonic_prost::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static("/hellas.Quote/GetQuote");
            let mut req = request.into_request();
            req.extensions_mut().insert(GrpcMethod::new("hellas.Quote", "GetQuote"));
            self.inner.unary(req, path, codec).await
        }
    }
}
...
pub mod quote_server {
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with QuoteServer.
    #[async_trait]
    pub trait Quote: std::marker::Send + std::marker::Sync + 'static {
        async fn get_quote(
            &self,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for QuoteServer<T>
    where
        T: Quote,
        B: Body + std::marker::Send + 'static,
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/hellas.Quote/GetQuote" => {
                    #[allow(non_camel_case_types)]
                    struct GetQuoteSvc<T: Quote>(pub Arc<T>);
                    impl<T: Quote> tonic::server::UnaryService<super::GetQuoteRequest>
                    for GetQuoteSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as Quote>::get_quote(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for QuoteServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "hellas.Quote";
    impl<T> tonic::server::NamedService for QuoteServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
