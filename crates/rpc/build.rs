...
fn render_client_traits(services: &[RpcService]) -> String {
    ...
    for service in services {
        let trait_name = format!("{}Client", service.name);
        let feature = feature_for_package(&service.package);
        let cfg = format!("#[cfg(all(feature = \"{feature}\", feature = \"iroh-client\"))]");
        ...
        for method in &service.methods {
            ...
            let call_ty = call_type_for(method, &marker_ident);
            output.push_str(&format!(
        ...
        for method in &service.methods {
            ...
            let call_ty = call_type_for(method, &marker_ident);
            let call_ctor = call_ctor_for(method, &marker_ident);
            output.push_str(&format!(
                "    fn {fn_name}(&self, request: {request_arg}) -> {call_ty} {{\n\
                     {call_ctor}\n\
                 }}\n",
...
fn call_type_for(method: &RpcMethod, marker_ident: &str) -> String {
    let prefix = call_prefix(method);
    format!("crate::call::{prefix}<crate::service::methods::{marker_ident}>")
}

fn call_ctor_for(method: &RpcMethod, marker_ident: &str) -> String {
    let prefix = call_prefix(method);
    format!(
        "        crate::call::{prefix}::<crate::service::methods::{marker_ident}>::new(self.clone(), request)"
    )
}
